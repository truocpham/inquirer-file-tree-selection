...
class FileTreeSelectionPrompt extends Base {
  ...
  renderFileTree(root = this.fileTree, indent = 2) {
    ...
    children.forEach((itemPath) => {
      ...
      if (this.opt.multiple) {
        prefix += this.selectedList.includes(itemPath.path)
          ? figures.radioOn
          : figures.radioOff;
        prefix += ' ';
      }
  ...
  async prepareChildren(node) {
    ...
    const onValid = this.opt.onValid;
    if (onValid) {
      const addValidity = async (fileObj) => {
        const isValid = await onValid(fileObj.path);
   ...
   * @return {FileTreeSelectionPrompt} this
   */
  ...
  render(error) {
    ...
    if (this.firstRender) {
      message += chalk.dim(
        '(Use arrow keys, Use space to toggle folder, Use tab to expand folder)'
      );
    }

    if (this.status === 'answered') {
      message += chalk.cyan(
        this.opt.multiple ? this.selectedList.join(', ') : this.active.path
      );
    } else {
      this.shownList = [];
    ...
    if (error) {
      bottomContent = '\n' + chalk.magenta('> ') + chalk.magenta(error);
    }
    ...
    if (!triggerByTab && this.opt.multiple) {
      ...
      if (this.selectedList.includes(this.active.path)) {
        this.selectedList.splice(
          this.selectedList.indexOf(this.active.path),
          1
        );
      } else {
